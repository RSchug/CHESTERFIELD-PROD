/*------------------------------------------------------------------------------------------------------/
| Program: BatchPlanningLDSExpired  Trigger: Batch (Nightly)
| Client : Chesterfield County
|
| Version 1.0 - Ray Schug - TruePoint Solutions
|
|   Planning/Limited Duration Sign/NA/NA
|   0 days before Permit Expiration 
|   When Data field 'Removal Date' is  today's date then
|       Update Record Status to 'Expired' and all open Workflow Tasks should be set to closed .
|       Use the setTask("Application Submittal -Review","N","Y")
|       and setTask("Permit Issuance","N","Y")  since only 2 workflow tasks on this record type.
/------------------------------------------------------------------------------------------------------*/
...
//Parameter variables
var paramsOK = true;
var searchCapType = "Planning/Limited Duration Sign/*/*" // Cap Type(s) that should be processed.
...
var searchAppStatusInvalid = ["Completed", "Cancelled", "Denied", "Expired", "Withdrawn"];

var searchAppSubGroupName = "CC-PLN-LDS";                                      // Application Spec Info Subgroup Name that the ASI field is associated to.
var searchAppSpecInfoLabel = "Removal Date";                                   // ASI field name that the batch script is to search.

//var searchDateFrom = dateAdd(startDate, 150);
//var searchDateTo = dateAdd(searchDateFrom, 30);
var searchDateFrom = dateAdd(startDate, -5);    // 5 day catch up
...
function mainProcess() {
    ...
    */
    ...
    for (i in capIdArray) {
        ...
        for (var ww in tasks) {
            var wfTask = tasks[ww];
            //logDebug(ww + ": Check, Active: " + tasks[ww].active + ", Status: " + tasks[ww].status);
            if (tasks[ww].active == "Y") {
                logDebug(ww + ": Completing");
                setTask(ww, "N", "Y")
            }
        }
        if (capStatusNew)
