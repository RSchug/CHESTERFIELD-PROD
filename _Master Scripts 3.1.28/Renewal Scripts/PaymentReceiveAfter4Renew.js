		...
		if (result.getSuccess()) {
			...
			if (projectScriptModel.RENEWAL_COMPLETE.equals(projectScriptModel.getStatus())) {
				...
				if (activeLicense(parentLicenseCAPID)) {
					...
					//remove existing cap contacts from license 
					removeContactsFromCap(parentLicenseCAPID);
			...
			logDebug("ERROR: Failed to create renewal CAP : MasterCAP(. " + parentLicenseCAPID + ")  renewal CAP(" + capID + ")" + result.getErrorMessage());
		}
	}
}
